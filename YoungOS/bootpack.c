/*
 * @Author: Z. Young
 * @Date: 2020-07-07 20:01:38
 * @LastEditors: Z. Young
 * @LastEditTime: 2020-08-04 21:22:16
 * @Descripttion: 主程序文件
 * @version: 1.30.0.0
 */ 
#include "bootpack.h"
...
void show_systime(struct SHEET *sht, int scrnx, int scrny);
...
void Main(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	init_screen8(buf_back, binfo->scrnx, binfo->scrny, "bk1.jpg");
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		...
		show_systime(sht_back, binfo->scrnx, binfo->scrny);
		io_cli();
...
{
	char s[12];
	sprintf(s, "%d/%02d/%02d", get_year(),get_mon_hex(), get_day_of_month());
	putfonts8_asc_sht(sht, scrnx - 210, scrny - 20, COL8_000000, COL8_C6C6C6, s,10);
	int weekday = get_day_of_week();
	switch (weekday)
	{
		case 0:
			sprintf(s, "Sunday");
	...
	putfonts8_asc_sht(sht, scrnx - 117, scrny - 20, COL8_000000, COL8_C6C6C6, s,10);

	sprintf(s, "%2d:%02d", get_hour_hex(), get_min_hex());
	putfonts8_asc_sht(sht, scrnx - 45, scrny - 20, COL8_000000, COL8_C6C6C6, s,10);
	sheet_refresh(sht, scrnx - 200, scrny - 20, scrnx - 5, scrny - 34);
}
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	sheet_setbuf(sht, buf, 640, 480, 255); /*无透明色*/
	make_window8(buf, 640, 480, "Console", 0);
	make_textbox8(sht, 8, 28, 624, 448, COL8_000000);
