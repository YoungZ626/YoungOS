/*
 * @Author: Z. Young
 * @Date: 2020-07-26 15:10:00
 * @LastEditors: Z. Young
 * @LastEditTime: 2020-08-04 11:05:28
 * @Descripttion: 
 * @version: 
 */ 
#include "console.h"
...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	if (nihongo[4096] != 0xff) {	 /* 是否载入了字库？*/
		task->langmode = 3;
		cons_putstr0(&cons, "Welcome to YoungOS!!\n");
		cons_putstr0(&cons, "Chinese fonts are available. Have a good time!\n\n");
	} else {
		task->langmode = 0;
	}
	task->langbyte1 = 0;
...
void cons_newline(struct CONSOLE *cons)
{
	...
	if (cons->cur_y < 28 + 427) {
		...
		for (y = 28 + 432; y < 28 + 484; y++) {
			for (x = 8; x < 8 + 624; x++) {
				sheet->buf[x + y * sheet->bxsize] = COL8_000000;
			}
		}
		sheet_refresh(sheet, 8, 28, 8 + 624, 28 + 448);
	}
	cons->cur_x = 8;
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal)
{
	...
	if (strcmp(cmdline, "mem") == 0) {
		...
		if (cmd_app(cons, fat, cmdline) == 0) {
			/*不是命令，不是应用程序，也不是空行*/
			if(task->langmode == 3)
			cons_putstr0(cons, "该命令无法识别，请重新输入\n\n");
			else
